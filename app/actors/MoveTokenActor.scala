...
class MoveTokenActor @Inject() (gameStateDao: GameStateDao, movePlayerService: MovePlayerService) extends Actor {
  ...
  override def receive: Receive = {
    ...
    case MoveToken(streamId, playerId) =>
      val result: Future[MoveTokenResults] = gameStateDao.find(streamId).flatMap { gs =>
        gs match {
          case Some(gameState) =>
  ...
  private def moveComputer(streamId: String, playerId: String): Future[MoveTokenResults] = {
    gameStateDao.find(streamId).flatMap { gs =>
      gs match {
        case Some(gameState) =>
          ...
          movePlayerService.movePlayer(computer.identifier, gameState.copy(player = players))
        case None => Future(GameDoesNotExist())
      }
    }
  }
